...
class Server():
    ...
    def login_proto(self, Message, connection_from_client):
        ...
        N1 = os.urandom(16)
        N1 = N1.hex()
        self.logger.debug(f"{connection_from_client}:: Received N1")
        ...
        self.logger.debug(f"Sending from: {connection_from_client.getsockname()}")
        ...
        if Message.N1 == N1:
            self.logger.debug(f"{connection_from_client}:: Puzzle resolved!")
        else:
            self.logger.error(f"{connection_from_client}:: Puzzle not resolved!")
        ...
        self.logger.debug(f"{connection_from_client}:: Shared secret is:{int.from_bytes (Kas, sys.byteorder)}")
        Message.gb_mod_p = str (bob.public_key)
        ...
        authenticate_data = b'Final Project' # self.Message.authenticate_data = authenticate_data
        ...
        if ((int(time.time()) - message_timestamp) < 60):
            self.logger.debug(f"{connection_from_client}:: Timestamp verified!")
        else:
            self.logger.error(f"{connection_from_client}:: Timestamp failed!")
    ...
    def __init__(self):
        try: 
            ...
            self.logger.debug(f"Server listening socket created: {self.socket_from_client}")
            serveraddress = ('localhost',9090)
            self.socket_from_client.bind(serveraddress)
            self.logger.debug(f"Server listening socket bound to: {serveraddress}")
            self.socket_from_client.listen(1)
            while True:
                Message = COMM_MESSAGE()
                connection_from_client, client_address = self.socket_from_client.accept()
                data, temp = connection_from_client.recvfrom(4095)
                Message.ParseFromString(data)
